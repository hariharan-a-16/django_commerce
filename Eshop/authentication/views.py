...
def send_otp_mail(request):
    ...
    if request.method == 'POST':
        ...
        if email:
            ...
            message= f"Your OTP is {otp} . It will expired in 10 minutes"
            send_mail(
                subject=subject,
                message=message,
    ...
    return render(request,template_name='authentication/pwd_reset/send_otp_email.html',
                      context=context)
...
def verify_otp(request):
    ...
    if not email:
        return redirect('send_otp')  # safeguard
